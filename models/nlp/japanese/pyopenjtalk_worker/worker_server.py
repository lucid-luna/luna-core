# ====================================================================
#  File: models/nlp/japanese/pyopenjtalk_worker/worker_server.py
# ====================================================================
...
from models.nlp.japanese.pyopenjtalk_worker.worker_common import (
...
class WorkerServer:
    ...
    def start_server(self, port: int, no_client_timeout: int = 30) -> None:
        logger.info("[L.U.N.A.] pyopenjtalk worker server started")
            ...
            while True:
                if self.client_count == 0:
                    if no_client_since is None:
                        ...
                        logger.info("[L.U.N.A.] quit because there is no client")
                        return
                else:
                ...
                for sock in ready_sockets:
                    if sock is server_socket:
                        logger.info("[L.U.N.A.] new client connected")
                        ...
                        try:
                            ...
                            logger.info("[L.U.N.A.] close connection")
                            continue

                        logger.trace(f"[L.U.N.A.] server received request: {request}")

                        response = self.handle_request(request)
                        logger.trace(f"[L.U.N.A.] server sends response: {response}")
                        try:
                            send_data(sock, response)
                            logger.trace("[L.U.N.A.] server sent response successfully")
                        except Exception:
                            logger.warning(
                                "[L.U.N.A.] an exception occurred during sending response"
                            )
                        if self.quit:
                            logger.info("[L.U.N.A.] quit pyopenjtalk worker server")
                            return
